...
public class EditProductActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ProductDB myDB = new ProductDB(this);
        Product product = myDB.getProductById(productId);
        if (product != null) {
            name_input.setText(product.getName());
            color_input.setText(product.getColor());
            size_input.setText(product.getSize());
            brand_input.setText(product.getBrand());
            type_input.setText(product.getType());
            price_input.setText(product.getPrice());

            // Set the selection of the category, material, species, and water-resistant checkbox spinners
            String category = product.getCategory();
            category_spinner.setSelection(adapter.getPosition(category));

            String material = product.getMaterial();
            int materialIndex = materialAdapter.getPosition(material);
            if (materialIndex != -1) {
                material_spinner.setSelection(materialIndex);
            }

            String species = product.getSpecies();
            int speciesIndex = speciesAdapter.getPosition(species);
            if (speciesIndex != -1) {
                species_spinner.setSelection(speciesIndex);
            }

            String waterResistant = product.isWaterResistant();
            boolean isWaterResistant = Boolean.valueOf(waterResistant);
            water_resistant_checkbox.setChecked(isWaterResistant);
        ...
        update_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ProductDB myDB = new ProductDB(EditProductActivity.this);
                myDB.updateProduct(name_input.getText().toString().trim(),
